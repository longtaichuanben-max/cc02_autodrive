...
_TUNING_LOG_HEADER = [
    'time', 'cross_track_error', 'alpha_deg', 'steer_deg',
...
class PurePursuitController(Node):
    def __init__(self):
        ...
        self.declare_parameter('wp_radii', '5:1.2,7:2.0')             # 個別半径 例:"1:0.5,4:1.2"（skip前0-based）
        self.declare_parameter('speed_min', 1.5)           # m/s: セグメント最低速度
        self.declare_parameter('speed_max', 2.5)           # m/s: セグメント最高速度
        self.declare_parameter('speed_dist_short', 5.0)    # m: この距離以下でspeed_min
        self.declare_parameter('speed_dist_long', 20.0)    # m: この距離以上でspeed_max
        self.declare_parameter('wheelbase_m', 0.267)       # m: 前後軸間距離（実測値）
        self.declare_parameter('lookahead_min', 1.0)       # m: ルックアヘッド最小距離
        self.declare_parameter('lookahead_fraction', 0.40) # Ld目標 = fraction × seg_len
        ...
        self.declare_parameter('max_speed_mps', 4.0)       # m/s: 安全上限
        self.declare_parameter('gnss_timeout_s', 2.0)
        self.declare_parameter('corner_wp_indices', '0,2,6,8')
        self.declare_parameter('wp_skip_indices', '3')
        self.declare_parameter('corner_slowdown_ratio', 0.5)       # コーナー速度 = seg_speed × ratio
        self.declare_parameter('corner_slowdown_base_dist', 5.0)   # m: speed_min時の減速開始距離
        ...
        self._corner_exit_total_dist     = 1.0   # ゼロ除算防止初期値
        self._corner_exit_start_speed    = 0.0
    ...
    def _compute_segment_params(self):
        ...
        for i in range(n - 1):
            e1, n1 = self.waypoints[i]
            e2, n2 = self.waypoints[i + 1]
            d = math.hypot(e2 - e1, n2 - n1)

            # 速度: d を [d_short, d_long] → [speed_min, speed_max] に線形マップ
            ...
            # T_h: Ld = fraction×d になるよう逆算。負にはクランプ
    ...
    def _control(self):
        ...
        if self._last_ctrl_x is not None and self._corner_exit_dist_remaining > 0:
            ...
            self._corner_exit_dist_remaining = max(
                0.0, self._corner_exit_dist_remaining - dist_moved
            )
        ...
        if dist_to_wp <= arrival_radius:
            ...
            if passed_idx in self._corner_wp_set:
                # 出口ランプ: 接近セグメント速度から次セグメント速度へランプ
                next_seg_speed, _ = self._seg_params()   # waypoint_index 更新後
                self._corner_exit_total_dist     = corner_dist
                self._corner_exit_dist_remaining = corner_dist
                self._corner_exit_start_speed    = corner_speed
                self.get_logger().info(
                    f'コーナーWP[{passed_idx}]通過 → 出口ランプ {corner_dist:.1f}m '
                    f'({corner_speed:.2f}→{next_seg_speed:.2f}m/s)'
                )
        ...
        if approaching_corner:
            # corner_dist→0 で seg_speed→corner_speed へ線形ランプ
            ...
            # 出口ランプ: start_speed→次セグメント seg_speed へ線形ランプ
            ratio     = max(0.0, min(1.0,
                           self._corner_exit_dist_remaining / self._corner_exit_total_dist))
            cmd_speed = self._corner_exit_start_speed + (1.0 - ratio) * (seg_speed - self._corner_exit_start_speed)
        else:
            cmd_speed = seg_speed

        cmd_speed = max(0.0, min(self.max_speed, cmd_speed))

        # Ld = Ld_min + T_h × cmd_speed（ランプ中も指令速度に追従）
        lookahead_dist = self.lookahead_min + seg_gain * cmd_speed
        if approaching_corner:
            # コーナー手前でLdがWPを越えないようキャップ（次区間への先読み防止）

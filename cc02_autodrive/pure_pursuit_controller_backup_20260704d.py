...
class PurePursuitController(Node):
    def __init__(self):
        ...
        #   WP0: WP8→WP0→WP1の旋回角≈3°（直進に近い）→ 減速不要のためコーナー対象外
        self.declare_parameter('corner_wp_indices', '2,6,8')
        # lh_ramp_wp_indices: 通過後にLdをlookahead_minに固定してランプ回復するWP（corner_wp_indicesの部分集合）
        #   WP6: WP6→WP7が12.18mで通常Ld=4.87m→steer=6.2°→92°ターン不可のため発動必須
        #   WP8: WP8→WP0が3.37mで通常もLd=2.0m（T_h=0）だが、lh_rampを保険として適用
        #   WP2: 旋回前後でLd=2.0m固定にすると旋回しすぎ（steer=13.8°）→ 対象外にして自然なLd≈2.7mを使う
    ...
    def _control(self):
        ...
        if self._last_ctrl_x is not None and (
            ...
            if self._speed_ramp_remaining > 0:
        ...
        if dist_to_wp <= arrival_radius:
            ...
            # lh_gain_ramp: 指定コーナーWP通過後のみLdをlookahead_minに固定してramp_dist区間で線形回復
            # WP6/WP8のみ発動 → WP2は発動しないので自然なLd（≈2.7m）でゆるやかにターン
        ...
        self._tuning_writer.writerow([
            ...
            f'{1 if self._speed_ramp_remaining > 0 else 0}',
            f'{1 if self._lh_gain_ramp_remaining > 0 else 0}',
